...
def recall_matched(reference, predicted, score_threshold=0.20):
    result=0
...
def predicted_matched(reference, predicted, score_threshold=0.20):
    result = 0
...
def maximum_matching_ratio(reference, predicted, score_threshold=0.20):
    scores = {}
...
class MatchApplication(object):
    ...
    def read_complexes(self, fname, known_proteins=None, strictness=0.5,
                       min_size=3, max_size=25,min_num=3,max_num=25):
        result = []
    ...
    def run(self):
        ...
        if self.options.network:
            data_ppi = "data/" + self.options.network + "/" + self.options.network + ".txt"
            known_proteins = self.read_network(data_ppi)
            self.log("%d known proteins found in network" % len(known_proteins))
        else:
            known_proteins = None

        
        reference_complexes = self.read_complexes("data/complex.txt", known_proteins)
        predicted_complexes = self.read_complexes(self.args[0], known_proteins)
